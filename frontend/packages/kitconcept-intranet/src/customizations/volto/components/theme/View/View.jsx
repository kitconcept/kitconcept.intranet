...
class View extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    // If returnUrl changed (including value changes, not just appear/disappear)
    if (currentReturnUrl !== prevReturnUrl) {
      // Dismiss old toast
      toast.dismiss('returnToOrigin');

      // Show new toast with slight delay to ensure dismiss completes
      if (currentReturnUrl) {
        setTimeout(() => {
          toast.info(
            <Toast
              info
              title="Back to original page"
              content="You opened this item from another page. Use this link to return."
            />,
            {
              autoClose: false,
              toastId: 'returnToOrigin',
              onClick: () => {
                this.props.history.replace({ pathname: currentReturnUrl });
                toast.dismiss('returnToOrigin');
              },
            },
          );
