...
Each block can have a width selected by the editor through the block width widget (see the {doc}`/developer/reference/registry` reference for details on the `blockWidth` widget).
